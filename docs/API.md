...
> 约定：
> - 前缀统一 `/api`；JSON 请求/响应；金额单位**分**；时间 ISO 8601。
...
```jsonc
// res
{ "items": [ { "id": "p1", "title": "贪吃蛇 3D", "category": "game", "priceCents": 9900, "coverUrl": "/api/files/p1/cover.png",
...
{ "id": "p1", "title": "贪吃蛇 3D", "description": "...", "category": "game",
  "priceCents": 9900, "trialScope": "前 3 关可玩", "coverUrl": "/api/files/p1/cover.png",
...
字段：`title` `description` `category` `priceCents`（0=免费）`trialScope` `cover`(图片,可选) `file`(html 或 zip，见 ARCHITECTURE §3.3 大小/解压边界)。
```jsonc
// res 201
...
更新元数据（title/description/category/priceCents/trialScope/cover）。`status` 不可在此修改。

### POST /api/projects/:id/submit — 作者本人
`draft → submitted`，进入审核队列。
```jsonc
// res 200  { "project": { "id": "p1", "status": "submitted" } }
```
...
```jsonc
// res
{ "status": "under review", "reviewNote": null, "submittedAt": "...", "reviewedAt": null, "history": [...] }
```
...
// res  { "project": { "id": "p1", "status": "delisted" } }
```

### POST /api/admin/projects/:id/approve · /reject — 审核员（角色待定，草案：admin 或 seller 内置）
```jsonc
// approve: {} → { "project": { "status": "approved", "publishedAt": "..." } }
// reject:  { "reviewNote": "入口页加载失败" } → { "project": { "status": "rejected" } }
```
...
// req  { "projectId": "p1" }
// res 201
{ "order": { "id": "o1", "orderNo": "VCM20260824XXXX", "priceCents": 9900, "feeCents": 495,
             "totalCents": 10395, "status": "pending", "escrowStatus": "none" } }
```

### GET /api/orders/:id/quote — buyer（或 GET /api/projects/:id/quote）
下单前查看实际应付总额（PRD 4：下单前一屏显示含手续费总价）。
```jsonc
// res  { "projectId": "p1", "priceCents": 9900, "feeCents": 495, "totalCents": 10395 }
```

### POST /api/orders/:id/pay — buyer
模拟支付：余额 ≥ totalCents 则扣款，钱进托管 `escrowStatus=held`，订单 `paid`。
```jsonc
// res 200
{ "order": { "id": "o1", "status": "paid", "escrowStatus": "held", "paidAt": "..." },
  "balanceAfterCents": 5000 }
```
涉及状态词：`pending → paid`、escrow `none → held`。

### POST /api/orders/:id/cancel — buyer
**仅未付款**可取消，一步完成、不追问原因：`pending → cancelled`。

### POST /api/orders/:id/refund — buyer
已付款订单退款：escrow `held → refunded`，余额全额退回，`paid → refunded`。退款入口在订单页常驻可见。
```jsonc
// res  { "order": { "status": "refunded", "escrowStatus": "refunded" }, "refundedCents": 10395, "balanceAfterCents": 15000 }
```
...
```jsonc
{ "items": [ { "id": "o1", "orderNo": "...", "project": {...}, "priceCents": 9900, "totalCents": 10395,
...
```jsonc
// req
{ "title": "帮我做一个课堂小游戏", "description": "...", "budgetMinCents": 5000, "budgetMaxCents": 15000,
...
{ "commission": { "id": "c1", "status": "open", "acceptanceCriteria": "1) ...", "criteriaHash": "sha256:...",
                  "budgetMinCents": 5000, "budgetMaxCents": 15000, "timelineDays": 7, "createdAt": "..." } }
```

### GET /api/commissions — 公开（需求板）
筛选：`?status=open&budgetMaxLte=10000&sort=newest&q=&page=`
```jsonc
{ "items": [ { "id": "c1", "title": "...", "budgetMinCents": 5000, "budgetMaxCents": 15000, "timelineDays": 7,
...
{ "id": "c1", "title": "...", "description": "...", "acceptanceCriteria": "1) ...", "criteriaHash": "...",
  "budgetMinCents": 5000, "budgetMaxCents": 15000, "timelineDays": 7, "referenceProjects": [...],
  "status": "open", "bids": [ { "id": "b1", "contractor": {...}, "amountCents": 8000, "status": "submitted" } ] }
```
...
投标（金额须在预算区间内；一人一单一标）。
```jsonc
// req  { "amountCents": 8000, "proposal": "我做过多款小游戏..." }
// res 201  { "bid": { "id": "b1", "amountCents": 8000, "status": "submitted" } }
```
...
```jsonc
// req  { "bidId": "b1" }
// res  { "contract": { "id": "k1", "status": "selected", "agreedAmountCents": 8000, "escrowStatus": "none" } }
```

### POST /api/contracts/:id/start — buyer（或 select 时自动）
预算进托管：buyer 扣 `agreedAmountCents` → 合同 `selected → in progress`、escrow `held`。
```jsonc
...
```jsonc
{ "id": "k1", "status": "in progress", "escrowStatus": "held", "agreedAmountCents": 8000,
...
### GET /api/wallet — 登录
```jsonc
{ "balanceCents": 12000, "escrowHeldCents": 8000,   // 托管中的钱（可一眼看出"钱在谁手里"）
  "currency": "CNY" }
```

### POST /api/wallet/topup — 登录（模拟支付）
```jsonc
// req  { "amountCents": 50000, "confirm": true }   // ≥阈值(500元)必须 confirm=true 二次确认，否则 400
// res  { "balanceAfterCents": 62000, "transaction": { "id": "t1", "type": "topup", "direction": "credit", "amountCents": 50000 } }
```

### GET /api/wallet/transactions — 登录
`?type=&direction=&page=`：
```jsonc
{ "items": [ { "id": "t1", "type": "escrow_release", "direction": "credit", "amountCents": 8000,
               "balanceAfterCents": 62000, "refType": "contract", "refId": "k1", "note": "接单《小游戏》结算入账", "createdAt": "..." } ], "page": 1, "total": 23 }
```

### POST /api/wallet/withdrawals — 登录
提现（模拟身份+银行卡校验）：`pending`，`etaDays` 展示到账时间（PRD 4：注明到账时间）。
```jsonc
// req  { "amountCents": 10000, "bankName": "测试银行", "cardLast4": "1234", "holderName": "小明" }
// res 201  { "withdrawal": { "id": "w1", "amountCents": 10000, "status": "pending", "etaDays": 1 } }
```

### GET /api/wallet/withdrawals — 登录
`?status=&page=` → `{ items: [{ id, amountCents, status: "pending"|"paid"|"rejected", etaDays, createdAt }] }`

### GET /api/wallet/escrow — 登录
托管总览（PRD 区域 6 两个必答问题：钱在谁手里 / 何时到账）。
```jsonc
{ "items": [ { "refType": "order", "refId": "o1", "direction": "in"|"out", "amountCents": 10395,
...
| 方法/路径 | 鉴权 | 说明 |
|-----------|------|------|
| GET /play/:projectId | 公开（仅 approved；作者可预览） | iframe 试玩入口，回放 uploads/projects/<id>/，带 CSP/sandbox 响应头（见 ARCHITECTURE §3.3） |
| GET /api/files/:projectId/* | 公开/登录按需 | 作品静态资源（cover 等） |
